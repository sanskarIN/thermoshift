...
17. Create the annotated semantic version tag `v2.8.2` only after the candidate is approved.
...
The manual hosted generator verifies version consistency, Tauri frontend paths, generated icon integrity, and documentation links before committing and stores candidate-SHA-qualified artifact evidence. Before pushing, it verifies that the remote branch has not advanced from the manifest revision used to generate the dependency graphs. If it has, regenerate on the new head instead of rebasing stale lockfiles.

For `2.8.2`, the lockfiles must be regenerated after all version/documentation candidate changes are complete so the final locked graph and exact-head verification correspond to the same candidate source.
