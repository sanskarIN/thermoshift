...
3. any intentional budget change with its reason;
4. browser/device profiling evidence when a performance issue motivated the release change.

Budgets must be changed in a reviewable commit and accompanied by documentation explaining why the previous limit is no longer appropriate. Do not raise limits only to make a failing build pass.
